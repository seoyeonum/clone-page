...
public interface ISitDAO
{
	...
	// [관리자] 시터 회원 리스트
	public List<SitDTO> list(@Param("start") int start
							 , @Param("end") int end
				             , @Param("searchKey") String searchKey
				             , @Param("searchValue") String searchValue);

	// [관리자] 전체 시터 회원 명수 확인
	public int countSit(@Param("searchKey") String searchKey
						, @Param("searchValue") String searchValue);
	
	// [관리자] 시터 회원가입 요청 코드로 상세정보 조회
	public SitDTO searchSitRegDetail(String sit_backup_id);
	
	// [관리자] 백업 아이디로 자기 정보 찾기
    public SitDTO adminSearchSitDetail(String sit_backup_id);
